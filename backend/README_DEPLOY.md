...
2. Create `.env` in `backend/` with:
   ...
   - For Postgres (Render/Aiven): `DATABASE_URL="postgres://<user>:<password>@<host>:<port>/<database>?sslmode=require"`
...
- For simple demos, you may keep `DATABASE_URL` as SQLite locally, but production should use Postgres.
- For production (Render, Railway, etc.), use Postgres:
  - Set `DATABASE_URL` to your Postgres URL with `?sslmode=require`
